  ...
  io::ShootMode shoot_mode, bool to_now)
{
  ...
  tools::logger()->info("CHECK control:{}",command.control);
...
//   // ===== AIMER EKF DEBUG =====
//   tools::logger()->warn(
//     "[AIMER DEBUG] w(rad/s)={:.3f}, r(m)={:.3f}",
//     ekf_x[7],   // angular velocity
//     ekf_x[8]    // radius
//   );
// // ==========================
  ...
  // 不考虑小陀螺
  if (std::abs(target.ekf_x()[7]) <= 1.5 && target.name != ArmorName::outpost) {
    tools::logger()->warn(
    "[AIMER BRANCH] NORMAL (non-spin)  w={:.3f} rad/s, r={:.3f} m",
    ekf_x[7], ekf_x[8]
  );
    // 选择在可射击范围内的装甲板
    ...
    tools::logger()->warn(
    "[AIMER BRANCH] SPIN (top)          w={:.3f} rad/s, r={:.3f} m",
    ekf_x[7], ekf_x[8]
  );
